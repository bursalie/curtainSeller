...
namespace Amcacik.Tests
{
    public abstract class AmcacikTestBase : AbpIntegratedTestBase<AmcacikTestModule>
    {
        ...
        protected void UsingDbContext(Action<AmcacikDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AmcacikDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AmcacikDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AmcacikDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AmcacikDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AmcacikDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AmcacikDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AmcacikDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AmcacikDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AmcacikDbContext, Task<T>> func)
        {
            T result;
